  ...
  return (
    <Container>
      ...
      <Row className="bg-light p-4 rounded shadow">
        <Col md={3}>
          <Card.Img variant="top" src={`${API_BASE_EMPLOYEE_URL}/image/${imageURL}`} alt={title} style={{ width: "200px", height: "500px;", objectFit: "cover" }} />
        </Col>
        ...
        <Col md={6}>
          ...
          <p>
            <strong>Forma:</strong> {form}
          </p>
          <p>
            <strong>Pokój:</strong> {room.name} {room.number}
          </p>
          <p>
      ...
      <Row className="bg-light p-4 rounded shadow">
              ...
              {rowSeats.map((seat) => {
                ...
                return (
                  <Col key={seat.ID} xs="auto" className="p-1">
                    <Button variant={isSelected ? "warning" : isAvailable ? "success" : "danger"} onClick={() => handleSeatClick(seat.ID)} className="seat" disabled={!isAvailable}>
                      {seat.row}
                      {seat.number}
                    </Button>
                  </Col>
        ...
        <Col md={2}>
          ...
          <Link
            to={`/reservation-finalization/${showID}?date=${date}&price=${price}&form=${form}&movieTitle=${title}&cinemaName=${cinemaName}&selectedSeats=${selectedSeats}&selectedSeatsNames=${selectedSeatsNames}`}
          >
            <br></br>
