  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        ...
        await Promise.all(
          data_cinemasAndPrograms.map(async (cinema) => {
  ...
  return (
    <Container>
      ...
      <Row className="my-4">
        ...
        <Col md={3}>
          <Card.Img variant="top" src={`${API_BASE_EMPLOYEE_URL}/image/${movie.imageURL}`} alt={movie.title} style={{ width: "200px", height: "500px;", objectFit: "cover" }} />
        </Col>
      ...
      <Row className="my-4">
        <Col md={3}>
          <h4>Subskrypcje</h4>
          <p>
            Podoba ci się taki rodzaj filmu? <br></br> Zasubskrubyj aby być na bieżąco!
          </p>
          <Form>
              ...
              {selectedCinema && (
                <ul className="mt-3">
                  {(() => {
                    ...
                    const selectedPrograms = cinemasAndPrograms.filter((cinema) => cinema.name === selectedCinema);
                    ...
                    const filteredShows = movieShows.filter((showArray) => showArray.some((show) => selectedPrograms.some((program) => program.ProgramID === show.ProgramID)));
                    ...
                    return filteredShows.length > 0 ? (
                      filteredShows.flat().map((show) => (
                        <li key={show.ShowID} className="list-group-item d-flex justify-content-between align-items-center">
                          ...
                          <Link
                            to={`/movie-reservation/${show.ShowID}?date=${show.Date}&price=${show.Price}&form=${show.Form}&movieTitle=${movie.title}&roomID=${show.RoomID}&cinemaName=${selectedCinema}&imageURL=${movie.imageURL}`}
                          >
